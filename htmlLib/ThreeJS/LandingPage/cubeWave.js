...
for (let i = 0; i < cubeAmount; i += 1)
{
    let tPos = lerp(cubePath.startTPos, cubePath.endTPos, i / cubeAmount); // Distribute tPos equally
    tPos -= cubePath.endTPos // Move cubes bejoind start position, so they fly in initially

    let pathCube = new Cube(i, tPos);
    pathCube.randomizeMesh();

    pathCubes.push(pathCube);
    scene.add(pathCubes[i].mesh);
}
